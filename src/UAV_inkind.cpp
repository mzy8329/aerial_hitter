...
UAV_inkind::UAV_inkind(ros::NodeHandle nh, double ctrl_freq, double UAV_vel)
{
    _nh = nh;
    ...
    _hitPoint_pub = nh.advertise<nav_msgs::Odometry>("/UAV_inkind/hitPoint", 1);

    _hitingAllow_service = nh.advertiseService("/UAV_sim/hitingAllow", &UAV_inkind::hitingAllow_Callback, this);
...
void UAV_inkind::state_Callback(const mavros_msgs::StateConstPtr &msg)
{
    _current_state = *msg;
}
...
void UAV_inkind::initParam(ros::NodeHandle nh)
{
...
void UAV_inkind::Land()
{
    
}


void UAV_inkind::printData()
{
    switch (_mode)
    {
    case wait:
...
void UAV_inkind::hit2base()
{
    ...
    double tan0 = tan(_Arm.Arm._motor[0].angle_fdb + _Arm.arm_offset[0]);
...
template<typename T>
bool UAV_inkind::checkSafeBox(T pt)
{
