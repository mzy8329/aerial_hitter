#include "sim_uav_base.h"


sim_uav_base::sim_uav_base(ros::NodeHandle nh, double ctrl_freq, double UAV_vel)
{
    _nh = nh;
    ...
    _state_sub = nh.subscribe("/mavros/state", 10, &sim_uav_base::state_Callback, this);
    _target_pose_sub = nh.subscribe("/sim_uav_base/target_pose", 1, &sim_uav_base::targetPose_Callback, this);
    _current_pose_sub = nh.subscribe("/sim_uav_base/pose", 1, &sim_uav_base::currentPose_Callback, this);
    _ballPoseVrpn_sub = nh.subscribe("/vrpn_client_node/RigidBody3/pose", 10, &sim_uav_base::ballPoseVrpnCallBack, this);
    _ballPoseGazebo_sub = nh.subscribe("/ball_odom", 10, &sim_uav_base::ballPoseGazeboCallBack, this);
    ...
    _hitPoint_pub = nh.advertise<nav_msgs::Odometry>("/sim_uav_base/hitPoint", 1);
...
void sim_uav_base::state_Callback(const mavros_msgs::StateConstPtr &msg)
{
    _current_state = *msg;
}

void sim_uav_base::currentPose_Callback(const geometry_msgs::PoseStampedConstPtr &msg)
{
    _currentPose = *msg;
}

void sim_uav_base::targetPose_Callback(const trajectory_msgs::MultiDOFJointTrajectoryConstPtr &msg)
{
    _targetPose = *msg;
}

void sim_uav_base::ballPoseVrpnCallBack(const geometry_msgs::PoseStampedConstPtr &body_msg)
{
    static int i_1 = 0;
    static int i_2 = 0;
...
void sim_uav_base::ballPoseGazeboCallBack(const nav_msgs::OdometryConstPtr &body_msg)
{
    static int i_1 = 0;
    static int i_2 = 0;
    Eigen::Vector4d point;
...
void sim_uav_base::initParam(ros::NodeHandle nh)
{
        ...
        && _Predict.trajPredict.predictTraj_hit(k, _point_target))
        {
            ...
            else
            {
                ...
                for(int i = 0; i < 2; i++)
                {
                    ...
                    _Arm.arm_pos_target[i] =  common_tools::triangleProfile(P_s, P_e, P_p, 1.0/_ctrl_rate);
                }   
                _mode = hit;
            }            
        }
...
{    
    ...
    if(time_now - _Arm.arm_pos_target[0][0][2] > 1.0/_ctrl_rate)
    {
        ...
        if(_Arm.arm_pos_target[1].size() > 0)
        {
            ...
            _Arm.arm_pos_target[1].erase(_arm_pos_target[1].begin());
        }
...
void sim_uav_base::hit2base()
{
    ...
    double p2 = (a*_Arm.arm_length[0]*(Pa_h[2]-Pb_k[2]) + b*(p2_k-sqrt(m1_h*m1_h+n1_h*n1_h)))/(a*_arm_length[0]*_arm_length[0] + b);
...
template<typename T>
T sim_uav_base::checkSafeBox(T pt)
{
